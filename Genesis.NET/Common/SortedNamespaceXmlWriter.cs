...
class NamespaceElement {
    ...
    public NamespaceElement(string prefix, string uri)
    {
        this.prefix = prefix;
        this.uri = uri;
    }
...
class SortedNamespaceXmlWriter : XmlWriter
{
    ...
    public SortedNamespaceXmlWriter(XmlWriter baseWriter) {
    ...
    {
        if (_inXmlNsAttribute)
            _namespaces.Add(new NamespaceElement(_nsPrefix, text));
        else
    ...
    {
        if (_elementIndex != 2 || _namespaces.Count == 0)
            return;

        _namespaces.Sort((a, b) => StringComparer.Ordinal.Compare(a.prefix, b.prefix));
        _namespaces.Reverse();

        foreach (var element in _namespaces)
        {
            _baseWriter.WriteStartAttribute("xmlns", element.prefix, null);
            _baseWriter.WriteString(element.uri);
            _baseWriter.WriteEndAttribute();
        }
